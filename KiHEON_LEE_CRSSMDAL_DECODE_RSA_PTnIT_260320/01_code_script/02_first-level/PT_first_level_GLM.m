%% PT first-level GLM specification, estimation, beta export, and contrast generation
% This script specifies and estimates the perception-task first-level GLM,
...
%
% By default, temporal high-pass filtering (128s) is handled within SPM during
% the first-level model specification.
...
analysis_path = 'first_level_GLM_for_perception';
...
n_runs = 12;
run_dirs = arrayfun(@(x) sprintf('01_FEPT_RUN%02d', x), 1:n_runs, 'UniformOutput', false);
...
%% Load firm-time tables used for rating durations
firm_v1 = importdata(fullfile(data_path, 'onset', 'visit1_data_firmtime.csv'));
firm_v2 = importdata(fullfile(data_path, 'onset', 'visit2_data_firmtime.csv'));
firm_v1(firm_v1 == -1000) = 5000;
firm_v2(firm_v2 == -1000) = 5000;
firm_v1 = firm_v1 / 1000;
firm_v2 = firm_v2 / 1000;
ptr_v1 = 1;
ptr_v2 = 1;
...
for ni = 1:n_runs
    ...
    for e = 1:numel(conditions)
        ...
        onset_file = fullfile(data_path, 'onset', sprintf('%s_output_onset_face_%s.csv', visit, cond_name));
        block_file = fullfile(data_path, 'onset', sprintf('%s_output_onset_face_%s_block.csv', visit, cond_name));
        intensity_file = fullfile(data_path, 'onset', sprintf('%s_output_onset_face_%s_intensity.csv', visit, cond_name));

        temp_onset = importdata(onset_file);
        temp_block = importdata(block_file);
        temp_intensity = importdata(intensity_file);

        % Retain trials with valid ratings only (1-7).
        idx = (temp_block == run_num) & (temp_intensity >= 1 & temp_intensity <= 7);
...
for ni = 1:n_runs
    ...
    % Rating period regressor: onset is 6.5 s after stimulus onset.
    rating_onsets = [];
    ...
    if ~isempty(rating_onsets)
        ...
        if ni <= 6
            rating_durs = firm_v1(ptr_v1 : ptr_v1 + n_trial - 1);
            ptr_v1 = ptr_v1 + n_trial;
        else
            rating_durs = firm_v2(ptr_v2 : ptr_v2 + n_trial - 1);
            ptr_v2 = ptr_v2 + n_trial;
        end
